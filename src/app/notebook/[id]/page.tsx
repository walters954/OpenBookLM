    ...
    useEffect(() => {
        const fetchNotebook = async () => {
            try {
                ...
                // First, fetch the notebook data
                const notebookResponse = await fetch(
                    `/api/notebooks/${params.id}`
                );
                const notebookData = await notebookResponse.json();

                if (!notebookResponse.ok) {
                    throw new Error(
                        notebookData.error || "Failed to load notebook"
                    );
                }

                setNotebook(notebookData);

                // Then try to fetch chat history, but don't fail if it doesn't work
                try {
                    const chatResponse = await fetch(
                        `/api/chat/history?notebookId=${params.id}`
                    );
                    ...
                    if (
                        ...
                        console.log(
                            "Successfully loaded chat history (nested format):",
                        ...
                        console.log(
                            "No valid chat history found, using empty message array"
                        );
                        setMessages([]);
                    }
                } catch (chatError) {
                    console.error(
                        "Error loading chat history (non-critical):",
                        chatError
                    );
                    // Don't fail the whole page load for chat history issues
                    setMessages([]);
                }
                ...
                setError(
                    error instanceof Error
                        ? error.message
                        : "An error occurred while loading the notebook"
                );
                setNotebook(null);
            } finally {
                setLoading(false);
            ...
            <NotebookHeader
                ...
                onTitleUpdate={async (newTitle) => {
                    try {
                        ...
                        console.error(
                            "Failed to update notebook title:",
                            error
                        );
                    }
                }}
